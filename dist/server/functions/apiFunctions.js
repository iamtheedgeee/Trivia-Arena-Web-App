...
import { AnswerCheckList, FreeResponseListSchema } from "../schema/questions.js";
...
export const getDummyQuestions = async (x) => {
    let dummy = [];
    dummy = [
        {
            sn: 1,
...
export const getQuestions = async (category) => {
    ...
    try {
        ...
        if (!raw)
            throw new CustomError("No response");
        //Extracting the Json Part for safe Parsing
        const jsonMatch = raw.match(/\[[\s\S]*\]/);
        if (!jsonMatch)
            throw new CustomError("No Json");
        ...
        if (!result.success)
            throw new CustomError("Json Shape Mismatch");
...
export const judgeAnswers = async (question, category) => {
    const verdict = [];
    const prompt = `
...
export const dummyJudge = async (question, x) => {
    const verdict = [];
    question.answers.forEach((answerObj) => {
        verdict.push({
            player: answerObj.player,
